...
//
// This file is a modified copy of the original ControlAffinePlantInversionFeedForward
// from WPILib that adds the capability for a dynamic B matrix
 ...
 * form f(x) + B(x)u).
 *
 * <p>The feedforward is calculated as <strong> u_ff = B(x)<sup>+</sup> (rDot - f(x))</strong>,
 * where <strong> B(x)<sup>+</sup> </strong> is the pseudoinverse of B(x).
 *
 * <p>This feedforward does account for a dynamic B matrix, B is calculated on every call to
 * calculate.
 *
 * <p>For more on the underlying math, read
 * https://file.tavsys.net/control/controls-engineering-in-frc.pdf.
 */
public class PlantInversionFeedForwardOnCrack<States extends Num, Inputs extends Num> {
   ...
   */
  public PlantInversionFeedForwardOnCrack(
      Nat<States> states,
      Nat<Inputs> inputs,
  ...
  public Matrix<Inputs, N1> calculate(Matrix<States, N1> r, Matrix<States, N1> nextR) {
    ...
    var m_B =
        NumericalJacobian.numericalJacobianU(
            states, inputs, m_f, nextR, new Matrix<>(inputs, Nat.N1()));
