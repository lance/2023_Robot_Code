...
/**
 * Filter based of the WPILib slewrate limiter. Uses separate limits for the rate of change away
 * from zero and towards zero (accel, decel)
 */
public class SplitSlewRateLimiter {
  ...
  public double calculate(double newValue) {
    ...
    if (speedDelta >= 0)
      prevVal +=
          MathUtil.clamp(newValue - prevVal, -accelLimit * elapsedTime, accelLimit * elapsedTime);
    else
      prevVal +=
          MathUtil.clamp(newValue - prevVal, -decelLimit * elapsedTime, decelLimit * elapsedTime);
